...
public class ProarcDatabaseV10 extends DBDatabase {
    ...
    public static int upgradeToVersion11(
        ...
        if (currentSchemaVersion < VERSION) {
            ...
            currentSchemaVersion = ProarcDatabaseV9.upgradeToVersion10(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
    ...
    private static void upgradeDdl(ProarcDatabaseV11 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.lockObjectFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.unlockObjectFunction, script);
