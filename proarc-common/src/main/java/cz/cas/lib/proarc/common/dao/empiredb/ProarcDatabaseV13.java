...
/**
 * Database schema version 13. It adds user permission.
 *
 ...
 */
@Deprecated
public class ProarcDatabaseV13 extends DBDatabase {

    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(ProarcDatabaseV13.class.getName());
    /**
     * the schema version
     */
    ...
    public static int upgradeToVersion14(
        ...
        if (currentSchemaVersion < VERSION) {
            ...
            currentSchemaVersion = ProarcDatabaseV12.upgradeToVersion13(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
        ...
        ProarcDatabaseV14 schema = new ProarcDatabaseV14();
        try {
            schema.open(context);
    ...
    private static void upgradeDdl(ProarcDatabaseV14 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.importToProdFunction, script);
