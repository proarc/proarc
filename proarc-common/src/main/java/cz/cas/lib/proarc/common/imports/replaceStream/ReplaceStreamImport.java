...
import cz.cas.lib.proarc.common.imports.replaceStream.FileReader.ImportSession;
import java.io.File;
...
public class ReplaceStreamImport implements ImportHandler {
    ...
    private void ingest(ImportOptions importConfig) throws InterruptedException {
    ...
    private void ingestFile(File file, ImportOptions importConfig) {
        FileIngest ingest = new FileIngest(iSession);
        ingest.ingest(file, importConfig);
    }

    private void load(ImportOptions importConfig) throws Exception {
        File importFolder = importConfig.getImportFolder();
        List<File> importFiles = ReplaceStreamScanner.findFile(importFolder);
    ...
    public void consume(List<File> importFiles, ImportOptions context) throws InterruptedException {
        int index = 1;
        for (File file : importFiles) {
            if (Thread.interrupted()) {
                throw new InterruptedException();
    ...
    private void consumeFile(File file, ImportOptions context, int index) {
