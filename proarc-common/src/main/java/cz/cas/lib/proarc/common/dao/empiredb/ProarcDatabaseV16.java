...
public class ProarcDatabaseV16 extends DBDatabase {
    ...
    public static int upgradeToVersion17(
        ...
        if (currentSchemaVersion < VERSION) {
            ...
            currentSchemaVersion = ProarcDatabaseV15.upgradeToVersion16(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
    ...
    private static void upgradeDdl(ProarcDatabaseV17 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);
            DBSQLScript script = new DBSQLScript(context);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.importToCatalogFunction, script);
