...
/**
 * Database schema version 8. It adds column changeModelFunction in UserTable
 *
 * @author Lukas Sykora
 */
@Deprecated
public class ProarcDatabaseV8 extends DBDatabase {
    ...
    public static int upgradeToVersion9(
        ...
        if (currentSchemaVersion < VERSION) {
            currentSchemaVersion = ProarcDatabaseV7.upgradeToVersion8(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
    ...
    private static void upgradeDdl(ProarcDatabaseV9 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.changeModelFunction, script);
