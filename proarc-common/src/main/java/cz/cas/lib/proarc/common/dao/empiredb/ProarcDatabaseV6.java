 ...
 */
@Deprecated
public class ProarcDatabaseV6 extends DBDatabase {

    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(ProarcDatabaseV6.class.getName());
    /**
     * the schema version
     */
    ...
    public static int upgradeToVersion7(
            int currentSchemaVersion,
            DBContext context, EmpireConfiguration conf) throws SQLException {

        if (currentSchemaVersion < VERSION) {
            currentSchemaVersion = ProarcDatabaseV5.upgradeToVersion6(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
        ...
        ProarcDatabaseV7 schema = new ProarcDatabaseV7();
        try {
            schema.open(context);
    ...
    private static void upgradeDdl(ProarcDatabaseV7 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableWorkflowTask.order, script);
