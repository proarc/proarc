...
public class ProarcDatabaseV6 extends DBDatabase {
    ...
    public static int upgradeToVersion7(
        ...
        if (currentSchemaVersion < VERSION) {
            currentSchemaVersion = ProarcDatabaseV5.upgradeToVersion6(currentSchemaVersion, conn, conf);
        }
        if (currentSchemaVersion > VERSION) {
...
//            schema.close(conn);
    ...
    private static void upgradeDdl(ProarcDatabaseV7 schema, Connection conn) throws SQLException {
        try {
...
//            queueNumber = addColumn("QUEUE_NUMBER", DataType.DECIMAL, 0, true);
    ...
    static int schemaExists(ProarcDatabaseV6 db, Connection c) {
        try {
    ...
    private static void createSchema(ProarcDatabaseV6 db, Connection conn) throws SQLException {
