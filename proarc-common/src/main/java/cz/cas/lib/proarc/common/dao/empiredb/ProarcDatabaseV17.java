...
/**
 * Database schema version 17. It adds user permission.
 *
 ...
 */
@Deprecated
public class ProarcDatabaseV17 extends DBDatabase {

    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(ProarcDatabaseV17.class.getName());
    /**
     * the schema version
     */
    ...
    public static int upgradeToVersion18(
        ...
        if (currentSchemaVersion < VERSION) {
            ...
            currentSchemaVersion = ProarcDatabaseV16.upgradeToVersion17(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
        ...
        ProarcDatabaseV18 schema = new ProarcDatabaseV18();
        try {
            schema.open(context);
    ...
    private static void upgradeDdl(ProarcDatabaseV18 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);
            DBSQLScript script = new DBSQLScript(context);

            EmpireUtils.addTable(schema.tableUserSetting, context, script);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableBatch.software, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableBatch.itemUpdated, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableBatch.updated, script);
