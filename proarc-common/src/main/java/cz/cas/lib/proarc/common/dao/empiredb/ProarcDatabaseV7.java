...
/**
 * Database schema version 7. It adds element order to wf_task.
 *
 ...
 */
@Deprecated
public class ProarcDatabaseV7 extends DBDatabase {

    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(ProarcDatabaseV7.class.getName());
    /**
     * the schema version
     */
    ...
    public static int upgradeToVersion8(
        ...
        if (currentSchemaVersion < VERSION) {
            currentSchemaVersion = ProarcDatabaseV6.upgradeToVersion7(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
    ...
    private static void upgradeDdl(ProarcDatabaseV8 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableWorkflowPhysicalDoc.edition, script);
