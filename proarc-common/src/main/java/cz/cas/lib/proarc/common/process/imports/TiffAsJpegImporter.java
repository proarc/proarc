...
public class TiffAsJpegImporter implements ImageImporter {
    ...
    public BatchManager.BatchItemObject consume(FileSet fileSet, ImportProcess.ImportOptions ctx) {
        ...
        FileSet.FileEntry jpegEntry = findJpeg(fileSet);
        // check jpeg file
        if (jpegEntry == null) {
            return null;
        }
    ...
    private FileSet.FileEntry convertToTiff(FileSet.FileEntry jp, Configuration processorConfig, File tiff) throws IOException {
        ...
        return new FileSet.FileEntry(tiff);
    }

    public File getTiff(FileSet fileSet, ImportProcess.ImportOptions ctx) {
        FileSet.FileEntry jpegEntry = findJpeg(fileSet);
        // check jpeg file
        if (jpegEntry == null) {
            return null;
