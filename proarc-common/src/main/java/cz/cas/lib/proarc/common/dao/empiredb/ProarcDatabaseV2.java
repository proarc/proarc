...
public class ProarcDatabaseV2 extends DBDatabase {
    ...
    public static int upgradeToVersion3(
        ...
        if (currentSchemaVersion < VERSION) {
            currentSchemaVersion = ProarcDatabaseV1.upgradeToVersion2(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
    ...
    private static void upgradeDdl(ProarcDatabaseV3 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);
            // add UserTable columns
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableBatch.profileId, script);
            script.executeAll();
        } finally {
