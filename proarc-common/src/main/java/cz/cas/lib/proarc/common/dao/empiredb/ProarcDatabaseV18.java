...
public class ProarcDatabaseV18 extends DBDatabase {
    ...
    public static int upgradeToVersion19(
        ...
        if (currentSchemaVersion < VERSION) {
            ...
            currentSchemaVersion = ProarcDatabaseV17.upgradeToVersion18(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
    ...
    private static void upgradeDdl(ProarcDatabaseV19 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.changeObjectsOwnerFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.deviceFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.changePagesFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.wfCreateJobFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.createUserFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.updateUserFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.deleteUserFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.solrFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.deleteActionFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.allObjectsFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.prepareBatchFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.sysAdminFunction, script);
    ...
    public static class BatchTable extends EnhancedDBTable {
        ...
        public BatchTable(DBDatabase db) {
            ...
            software = addColumn("SOFTWARE", DataType.VARCHAR, 2000, false);
            ...
            priority = addColumn("PRIORITY", DataType.VARCHAR, 50, false);
            params = addColumn("PARAMS", DataType.VARCHAR, 10000, false);
    ...
    public static final class BatchItemTable extends EnhancedDBTable {
        ...
        public BatchItemTable(DBDatabase db) {
            ...
            pid = addColumn("PID", DataType.VARCHAR, 50, false);
    ...
    public static final class UserTable extends EnhancedDBTable {
        ...
        public UserTable(DBDatabase db) {
            ...
            timestamp = addTimestamp("TIMESTAMP");
            organization = addColumn("ORGANIZATION", DataType.VARCHAR, 100, false);
            role = addColumn("ROLE", DataType.VARCHAR, 100, false);
    ...
    public static final class WorkflowJobTable extends EnhancedDBTable {
        ...
        public WorkflowJobTable(DBDatabase db) {
            ...
            model = addColumn("MODEL", DataType.VARCHAR, 500, false);
            state = addColumn("STATE", DataType.VARCHAR, 100, true);
...
//            queueNumber = addColumn("QUEUE_NUMBER", DataType.DECIMAL, 0, true);
    ...
    public static final class WorkflowPhysicalDocTable extends EnhancedDBTable {
        ...
        public WorkflowPhysicalDocTable(DBDatabase db) {
            ...
            detail = addColumn("DETAIL", DataType.VARCHAR, 200, false);
            issue = addColumn("ISSUE", DataType.VARCHAR, 100, false);
            sigla = addColumn("SIGLA", DataType.VARCHAR, 6, false);
            volume = addColumn("VOLUME", DataType.VARCHAR, 100, false);
            year = addColumn("YEAR", DataType.VARCHAR, 100, false);
            edition = addColumn("EDITION", DataType.VARCHAR, 2000, false);
            setPrimaryKey(materialId);
        }
    }
