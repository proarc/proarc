 ...
 */
@Deprecated
public class ProarcDatabaseV12 extends DBDatabase {

    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(ProarcDatabaseV12.class.getName());
    /**
     * the schema version
     */
    ...
    public static int upgradeToVersion13(
        ...
        if (currentSchemaVersion < VERSION) {
            ...
            currentSchemaVersion = ProarcDatabaseV11.upgradeToVersion12(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
        ...
        ProarcDatabaseV13 schema = new ProarcDatabaseV13();
        try {
            schema.open(context);
    ...
    private static void upgradeDdl(ProarcDatabaseV13 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableBatch.params, script);
