...
public class WaveImporter implements ImageImporter {
    ...
    public WaveImporter(BatchManager ibm) {
        this.ibm = ibm;
    }
    ...
    private void createRelsExt(DigitalObjectHandler objHandler, File f, ImportProcess.ImportOptions ctx) throws DigitalObjectException {
    ...
    private FileSet.FileEntry findWave(FileSet fileSet, List<Object> suffix) {
    ...
    private FileSet.FileEntry findFlac(FileSet fileSet, List<Object> suffix) {
        for (FileSet.FileEntry entry : fileSet.getFiles()) {
            String mimetype = entry.getMimetype();
    ...
    private class Entry {
        private FileSet.FileEntry entry;
        private AudioMimeType mimeType;
        private String type;

        public Entry(FileSet.FileEntry entry, AudioMimeType mimeType, String type) {
            this.entry = entry;
            this.mimeType = mimeType;
            this.type = type;
        }

        public FileSet.FileEntry getEntry() {
            return entry;
        }

        public void setEntry(FileSet.FileEntry entry) {
            this.entry = entry;
        }
