...
public class MetsElement implements IMetsElement {
    ...
    private void validateDCMODS() throws MetsExportException {
        ...
        try {
            if ("3.8".equals(this.modsStream.get(0).getAttribute("version"))) {
                validationErrors = MetsUtils.validateAgainstXSD(modsDoc, ModsDefinition.class.getResourceAsStream("mods-3-8.xsd"));
            } else if ("3.6".equals(this.modsStream.get(0).getAttribute("version"))) {
