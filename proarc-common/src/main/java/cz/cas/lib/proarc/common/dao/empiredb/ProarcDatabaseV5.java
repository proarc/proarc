...
public class ProarcDatabaseV5 extends DBDatabase {
    ...
    public static int upgradeToVersion6(
        ...
        if (currentSchemaVersion < VERSION) {
            currentSchemaVersion = ProarcDatabaseV4.upgradeToVersion5(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
    ...
    private static void upgradeDdl(ProarcDatabaseV6 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);
            // chronicle: create role
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.role, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.organization, script);
