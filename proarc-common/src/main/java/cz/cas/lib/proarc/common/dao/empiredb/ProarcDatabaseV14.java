 ...
 */
@Deprecated
public class ProarcDatabaseV14 extends DBDatabase {
    ...
    public static int upgradeToVersion15(
        ...
        if (currentSchemaVersion < VERSION) {
            ...
            currentSchemaVersion = ProarcDatabaseV13.upgradeToVersion14(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
        ...
        ProarcDatabaseV15 schema = new ProarcDatabaseV15();
        try {
            schema.open(context);
    ...
    private static void upgradeDdl(ProarcDatabaseV15 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.czidloFunction, script);
