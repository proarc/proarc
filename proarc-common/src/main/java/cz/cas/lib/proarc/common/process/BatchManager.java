...
public class BatchManager {
    ...
    private static final Logger LOG = Logger.getLogger(BatchManager.class.getName());
    ...
    @Deprecated
    public static BatchManager getInstance(AppConfiguration config) {
        if (INSTANCE == null) {
...
//                batchView.setProfileId("SECRET");
    ...
    public List<Batch> findWaitingInternalBatches() {
        ...
        batchDao.setTransaction(tx);
        try {
            return batchDao.findWaitingInternalBatches();
        } finally {
            tx.close();
        }
    }

    public List<Batch> findInternalRunningBatches() {
        BatchDao batchDao = daos.createBatch();
        Transaction tx = daos.createTransaction();
        batchDao.setTransaction(tx);
        try {
            return batchDao.findIntenalRunningBatches();
        } finally {
            tx.close();
        }
    ...
    public List<BatchItem> findBatchItemObject(ImportProcess.ImportOptions ctx, String originalFilename) {
