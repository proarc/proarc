 ...
 * @author Jan Pokorsky
 */
public class KakaduCompress extends ExternalProcess {

    public static final String ID = "kdu_compress";
    ...
    public static void main(String[] args) throws Exception {
        if (args != null && args.length > 2) {
            ...
            ExternalProcess ep = new KakaduCompress(config, new File(args[1]), new File(args[2]));
            ep.run();
        }
    }

    public KakaduCompress(Configuration conf, File input, File output) {
        super(conf);
        this.input = input;
        this.output = output;
    }
