...
public class ProarcDatabaseV15 extends DBDatabase {
    ...
    public static int upgradeToVersion16(
        ...
        if (currentSchemaVersion < VERSION) {
            ...
            currentSchemaVersion = ProarcDatabaseV14.upgradeToVersion15(currentSchemaVersion, context, conf);
        }
        if (currentSchemaVersion > VERSION) {
    ...
    private static void upgradeDdl(ProarcDatabaseV16 schema, DBContext context) throws SQLException {
        try {
            context.getConnection().setAutoCommit(true);

            DBSQLScript script = new DBSQLScript(context);

            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableUser.wfDeleteJobFunction, script);
            context.getDbms().getDDLScript(DBDDLGenerator.DDLActionType.CREATE, schema.tableWorkflowJob.model, script);
