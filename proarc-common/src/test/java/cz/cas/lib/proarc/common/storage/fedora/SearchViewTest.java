...
import static cz.cas.lib.proarc.common.storage.FedoraTestSupport.assertItem;
...
public class SearchViewTest {
    ...
    public void testReadJsonResult() throws Exception {
        FedoraStorageSearchView instance = new FedoraStorageSearchView(storage);
        String json = "{\"results\":[{\"pid\" : \"p1\", \"k0\" : \"1\"}]}";
        FedoraStorageSearchView.Result result = instance.readResponse(json);
...
//        fedora.getClient().debug(true);
    ...
    public void testBuildQuery() {
        assertEquals("title~'*test*'", FedoraStorageSearchView.buildQuery(new StringBuilder(), "title", "test").toString());
        assertEquals("title~'*test*'", FedoraStorageSearchView.buildQuery(new StringBuilder(), "title", "  test  ").toString());
        // issue 220
        assertEquals("title~'*test test*'", FedoraStorageSearchView.buildQuery(new StringBuilder(), "title", "  test test  ").toString());
        assertEquals("title~'*test?s test???*'", FedoraStorageSearchView.buildQuery(new StringBuilder(), "title", "test's test???").toString());
        assertEquals("", FedoraStorageSearchView.buildQuery(new StringBuilder(), "title", "").toString());
        assertEquals("", FedoraStorageSearchView.buildQuery(new StringBuilder(), "title", "***").toString());

        assertEquals("label~'*test1*' title~'*test2*'", FedoraStorageSearchView.buildQuery(
                FedoraStorageSearchView.buildQuery(new StringBuilder(), "label", "test1"),
                "title", "test2").toString());
    }

    @Test
    public void testNormalizePhrase() {
        assertEquals("*query*", FedoraStorageSearchView.normalizePhrase("*query*"));
        assertEquals("*query*", FedoraStorageSearchView.normalizePhrase("*query"));
        assertEquals("*query*", FedoraStorageSearchView.normalizePhrase("query*"));
        assertEquals("*query*", FedoraStorageSearchView.normalizePhrase(" * query* **"));
        assertEquals("*", FedoraStorageSearchView.normalizePhrase(" ***"));
        assertEquals("*", FedoraStorageSearchView.normalizePhrase(""));
        assertEquals("*", FedoraStorageSearchView.normalizePhrase(null));
        assertEquals("*Stráž pokroku*", FedoraStorageSearchView.normalizePhrase("Stráž pokroku"));
    }
