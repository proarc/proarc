...
public class TechnicalXmlAesEditor implements DatastreamEditor, RefreshAction.Refreshable {

    private static final Logger LOG = Logger.getLogger(TechnicalXmlAesEditor.class.getName());
    ...
    public TechnicalXmlAesEditor(ClientMessages i18n) {
    ...
    private void refresh(boolean cleanHistory) {
        if (digitalObject != null) {
            if (digitalObject.getPid() != null) {
                ...
                TextDataSource.getTechnicalMetadataAes().fetchData(pidCriteria, new DSCallback() {
                    @Override
    ...
    private void saveImpl(BooleanCallback callback, String newXml) {
        TechnicalAesCustomDataSource.getInstance().saveXmlDescription(digitalObject, newXml, timestamp, new TechnicalAesCustomDataSource.DescriptionSaveHandler() {

            @Override
            protected void onSave(TechnicalAesCustomDataSource.DescriptionMetadata dm) {
                super.onSave(dm);
                refresh(false);
            ...
            protected void onValidationError() {
                ...
                SC.ask(i18n.SaveAction_Title(), msg, value -> {
                    // save again
                    if (value != null && value) {
                        TechnicalAesCustomDataSource.getInstance().saveXmlDescription(digitalObject, newXml, timestamp, this, true);
                    }
                });
            }
